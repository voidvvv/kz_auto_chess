---
title: refactor: screen-component-based-refactoring-deepened
type: refactor
status: active
date: 2026-03-11
...
# 重构：Screen类组件化重构（深度增强版）

## 概述

将当前 945 行代码的 `GameScreen` 类重构为基于组件的架构，降低复杂度，提高可维护性和可扩展性。本计划在原有brainstorm基础上，融合了大量Java游戏开发最佳实践研究。
...
- 集成现有 InputMultiplexer，优先级：热键 > UI > 世界输入
...
- 优化 ShapeRenderer 调用（使用 setAutoShapeType 减少批次数）
...
- **`GameScreen` 显示时**：所有组件初始化，事件系统就绪
- **`GameScreen` 隐藏时**：所有组件清理，事件系统停止分发
- **拖拽中切换 Screen**：`hide()` 中取消拖拽，清理状态
- **组件 dispose**：所有组件的资源正确释放
...
### 推荐模式：GameEventSystem 中心化管理

基于对现有 `ModelHolder` 模式的研究，推荐使用集中式事件系统：
...
**优势：**
- 与现有 DamageEventListenerHolder 模式一致
- 集中管理，避免所有组件共享同一个 Holder 实例
- 清晰的注册/注销接口
...
| 方案 | 说明 | 适用场景 |
|------|------|---------|
| **接口注入** | 构造函数传入，松耦合 | 组件替换灵活 |
| **直接引用** | 直接访问字段，高性能 | 高频访问场景 |

**推荐**：对高频访问（如渲染中访问单位列表）使用直接引用，其他场景使用接口。
...
当前 `CardRenderer` 可能存在性能问题，建议：

```java
public class OptimizedCardRenderer {
    private static ShapeRenderer sharedShapeRenderer;

    public static void renderBatch(
        ShapeRenderer shapeRenderer, SpriteBatch batch,
        List<Card> cards, float startX, float startY,
        ...
        for (int i = 0; i < cards.size(); i++) {
            ...
            Color bgColor = getCardColorByTier(card.getTier());

            shapeRenderer.set(ShapeRenderer.ShapeType.Filled);
            shapeRenderer.setColor(bgColor);
            shapeRenderer.rect(x, startY, cardWidth, cardHeight);
...
private void setupInput() {
    ...
    multiplexer.addProcessor(new InputAdapter() {
        ...
        public boolean touchDown(int screenX, int screenY, int pointer, int button) {
            ...
            // 处理游戏世界输入
            handleGameWorldTouch(worldPos);
            return true; // 允许其他处理器处理
        }
    });
}
```
...
- `pause()` 在游戏暂停时保存状态
- `resume()` 恢复动画和状态
- `dispose()` 释放所有资源（textures, fonts, pools）
- 组件间调用顺序：先 `pause()` 所有组件，再 `resume()`
...
| 增强领域 | 关键改进 |
|---------|---------|
| **事件系统架构** | 引入 GameEventSystem 中心化管理，与现有 ModelHolder 模式一致 |
| **类型安全设计** | 使用具体事件类型（DragStartedEvent, DroppedEvent 等）替代 Object |
| **数据依赖处理** | 引入 BattleDataProvider 接口，明确组件间数据访问方式 |
| **性能优化** | 事件分发器在 `show()` 中创建一次，避免每帧分配 |
| **Scene2D 优化** | ShapeRenderer 批量绘制，减少批次数 |
| **输入处理优化** | InputMultiplexer 明确优先级和冲突规则 |
| **组件生命周期** | 清晰的接口定义和最佳实践 |

### 新增参考来源

- [LibGDX Scene2D Documentation](https://libgdx.com/wiki/graphics/scene2d/scene2d-ui)
- [LibGDX Viewports Documentation](https://libgdx.com/wiki/graphics/viewports)
- [LibGDX Performance Guide](https://yair210.medium.com/the-libgdx-performance-guide)
- [Java Game Programming Patterns](https://gameprogrammingpatterns.com/component.html)
- [Object Pooling in Java Games](https://raizensoft.com/tutorial/object-pool-libgdx)
